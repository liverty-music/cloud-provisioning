...
| **Public DNS provider** | Single Cloudflare zone `liverty-music.app` is authoritative for all hostnames (dev records under `dev`/`api.dev`/`auth.dev` labels) | Single Cloudflare zone `liverty-music.app` is authoritative for all hostnames (apex + `api` + `auth`) | n/a | ✅ Mutable (Cloudflare API). Per `consolidate-public-dns-on-cloudflare`: Cloud DNS public zones removed; Cloudflare Registrar's apex-NS-lock makes single-provider the only way to eliminate dev/prod DNS-topology asymmetry. Google Cert Manager DNS-01 works against Cloudflare. |
| **Cloud DNS public zone count** | 0 (consolidated to Cloudflare) | 0 (consolidated to Cloudflare) | n/a | n/a — both envs use Cloudflare exclusively for public DNS |
| **Cloud DNS private zone count** | 2 (`asia-northeast2.sql.goog` for Cloud SQL PSC, `googleapis.com` for Private Google Access) | 2 (same as dev) | n/a | ✅ Mutable; Cloud DNS retained for private/internal zones where Cloudflare has no equivalent |
| **Certificate Manager hostnames** | `dev.liverty-music.app`, `api.dev.liverty-music.app`, `auth.dev.liverty-music.app` (all ACME DNS-01 challenges hosted in Cloudflare) | `liverty-music.app` (apex), `api.liverty-music.app`, `auth.liverty-music.app` (all ACME DNS-01 challenges hosted in Cloudflare) | n/a | ❌ Certificate `domains` field is immutable; replace requires new cert. Apex cert added by `consolidate-public-dns-on-cloudflare`. |
