...
3. **Broken inherit chain**: if the push-path job failed specifically because an inherit step could not resolve a parent digest (`tried :<before> and :main`), seed the chain by pushing any build-relevant change (frontend: a `src/**` / `package.json` edit; backend: any `**.go` / `go.mod` edit) to `main` — that forces a fresh build, creates a new `:main` image, and re-establishes the chain. Once the seed commit's push-path run succeeds, re-run the **original SHA's** push-path run (the inherit step will now resolve via the new `:main` fallback), then re-run the release matrix entry per step 2. The original release is recoverable without re-cutting.
