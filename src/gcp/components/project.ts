...
export class ProjectComponent extends pulumi.ComponentResource {
	...
	constructor(
		...
		this.folder = this.createOrganizationFolder(
			brandId,
			displayName,
			environment,
			gcpConfig,
	...
	private createOrganizationFolder(
		...
		if (environment === 'prod') {
			...
			const prodStackRef = new pulumi.StackReference(
				`${org}/${project}/prod`,
			)
			...
			return pulumi
				.output(folderOutput)
				.apply((folder) =>
					gcp.organizations.Folder.get(brandId, folder.id),
				)
		}
	}
